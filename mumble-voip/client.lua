		...
		if radioChannel > 0 then
			if CompareChannels(playerData, "radio", radioChannel) then -- Check if player is in the same radio channel as you
				if playerServerId ~= player then
					...
					if mumbleConfig.showRadioList then
						SendNUIMessage({ radioId = player, radioName = voiceData[player].radioName, radioTalking = value }) -- Set player talking in radio list
					end
				end
			end
		end
		...
		if initialised then
			...
			if mumbleConfig.radioEnabled then
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(0, mumbleConfig.controls.radio.key) and not IsPedSwimming(PlayerPedId()) and not IsPedSwimmingUnderWater(PlayerPedId()) and not exports['esx_policejob']:isHandcuffed() and not exports['esx_animations']:getCarry() then
						--if playerData.radio > 0 then
							...
							if mumbleConfig.showRadioList then
								SendNUIMessage({ radioId = playerServerId, radioName = voiceData[playerServerId].radioName, radioTalking = true }) -- Set client talking in radio list
							end
							...
							CreateThread(function()
								...
								if mumbleConfig.showRadioList then
									SendNUIMessage({ radioId = playerServerId, radioName = voiceData[playerServerId].radioName, radioTalking = false }) -- Set client talking in radio list
								end
					...
					if playerData.callSpeaker then -- Check if they have loud speaker on
						...
						for i = 1, #playerList do -- Get a list of all players within loud speaker range
							...
							if playerId ~= remotePlayerId then
								...
								local remotePlayerPed = Citizen.InvokeNative(0x43A66C31C68491C0, remotePlayerId)
